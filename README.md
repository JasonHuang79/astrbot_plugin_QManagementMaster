...
> **💡 范围参数（`/mute` `/kick` `/warn` 通用）**
> 默认按配置项 `default_scope` 执行（默认 `local` = 仅本群，更安全；可在插件配置里改为 `link` 恢复"旧行为=全组联动"）。在命令后加范围参数可覆盖默认值：
> ```
> /mute -a @张三 30m 刷屏         # 整组联动（覆盖默认的仅本群；同 -all/联动）
> /warn @张三 注意言行 -l         # 仅当前群（同 -local/本群；英文标记可放任意位置）
> /warn 本群 @张三 注意言行       # 仅当前群（中文标记须放在第一个参数位）
> /mute -g 123456,789012 @张三 30m 刷屏   # 仅指定群（须属于该联动组执行群）
> /mute -g123456 @张三 30m 刷屏           # 仅指定群（-g 连写，无空格）
> /warn 群:123456 @张三 注意言行          # 仅指定群（头部写法）
> ```
> 指定群不在联动组执行群内时会被忽略并提示；播报群消息会标注本次范围（仅本群/指定群）。
...
### v1.2.9 (2026-09-07)
- ✨ `/mute` `/kick` `/warn` 支持**范围参数**：`-l`/`-local`/`本群` 仅当前群；`-a`/`-all`/`联动` 整组；`-g 群号,群号`、`-g群号` 连写或 `群:群号` 仅指定群（须属于该联动组执行群）
- 🎛️ 新增配置项 `default_scope`：未指定范围参数时的默认执行范围，可选 `link`（整组联动，旧行为）/ `local`（仅本群，默认，更安全），可被命令范围参数覆盖
- 🔔 播报群消息在非全组执行时标注本次范围（仅本群/指定群）；中文标记仅识别命令名后第一个参数位，避免吞掉原因正文
